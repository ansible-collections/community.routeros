    ...
    ('app',): APIData(
        versioned=[
            ('7.22', '>=', VersionedAPIData(
                ...
                fields={
                    'app-store-url': KeyInfo(read_only=True),
                    'auto-update': KeyInfo(),
                    'category': KeyInfo(read_only=True),
                    'cmds': KeyInfo(read_only=True),
                    'containers': KeyInfo(read_only=True),
                    # 'copy-from': KeyInfo(write_only=True),
                    'custom': KeyInfo(read_only=True),
                    'default-credentials': KeyInfo(read_only=True),
                    'default-network': KeyInfo(read_only=True),
                    'description': KeyInfo(read_only=True),
                    'disabled': KeyInfo(),
                    'environment': KeyInfo(),
                    'extra-mounts': KeyInfo(),
                    'firewall-redirects': KeyInfo(),
                    'from-app-store': KeyInfo(read_only=True),
                    'hw-device-access': KeyInfo(),
                    'interface': KeyInfo(read_only=True),
                    'name': KeyInfo(read_only=True),
                    'network': KeyInfo(),
                    'project-page': KeyInfo(read_only=True),
                    'pvid': KeyInfo(),
                    'required-mounts': KeyInfo(),
                    'running': KeyInfo(read_only=True),
                    'status': KeyInfo(read_only=True),
                    'ui-url': KeyInfo(read_only=True),
                    'use-https': KeyInfo(),
                    'variables-to-use-in-environment': KeyInfo(read_only=True),
                    'yaml': KeyInfo(),
                },
            )),
